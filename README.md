...
```
### Gym environments
1. Place expert trajectories as generated by `<<<TODO which file>>>.py` in `./datasets` folder.
2. For training/testing, run:
```shell script
python train.py --name gym --dataset gym --env_name Circles-v0 --net_type infogail_mlp --latent_optimizer ohs --n_latent 3 --gpu_ids 0,1 --criterion mse --match_criterion mse 
```

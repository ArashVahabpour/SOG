...
class BlockCoordinateSearch:
    def __init__(self, opt, criterion_no_reduction, device, netG):
        """
    ...
    def optimize(self, real):
        ...
        for round_idx, block_idx in itertools.product(range(self.opt.n_rounds),
            ...
            if 'deconv' in self.opt.model:
                # (batch_size * latent_batch_size) x n_latent x 1 x 1
                new_z_r = new_z.reshape(self.opt.batch_size * self.opt.latent_batch_size, self.opt.n_latent, 1, 1)
            else:
